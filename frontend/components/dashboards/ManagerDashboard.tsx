    ...
    <div className="space-y-6">
      ...
      {selectedProject && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="bg-white rounded-lg shadow p-6 md:col-span-2">
            ...
            <p className="text-gray-600 mb-4 text-sm">
              {loadingData ? 'Loading…' : `${enquiries.length} enquiry(s) for ${selectedProject.name}`}
            </p>
            <div className="flex flex-wrap gap-3">
              <button
                onClick={() => openModalWith('enquiries')}
                disabled={loadingData}
                className="bg-indigo-600 text-white px-4 py-2 rounded-lg hover:bg-indigo-700 font-medium disabled:opacity-50 transition"
              >
                View Enquiries
              </button>
              <button
                ...
                disabled={loadingData}
                className="bg-green-600 text-white px-4 py-2 rounded-lg hover:bg-green-700 font-medium disabled:opacity-50 transition"
              >
                Create Enquiry
              </button>
            </div>
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
        <p className="text-sm text-blue-800">
          <strong>Note:</strong> As a manager, you can view stock and locations (read-only), and create enquiries to the warehouse or buyer.
        </p>
      </div>
          ...
          {opSuccess ? (
            ...
            <form onSubmit={(e) => void handleCreateEnquiry(e)} className="space-y-4">
              <p className="text-sm text-gray-500">
                Create an enquiry for stock you need, assigned to the warehouse or buyer.
              </p>
